...
class Noise {
  ...
  public static noiseFunctions = [Noise.valueFunctions, Noise.perlinFunctions];

  static sqrt2 = Math.sqrt(2);
  ...
  public static value1d(p: Vector2, frequency: number) {
    ...
    return lerp(h0, h1, t) * (1 / Noise.hashMask);
  }
  public static value2d(p: Vector2, frequency: number) {
    ...
    return (
      lerp(lerp(h00, h10, tx), lerp(h01, h11, tx), ty) * (1 / Noise.hashMask)
    );
  }
  ...
  public static perlin2d(p: Vector2, frequency: number) {
    ...
    let h1 = Noise.hashTable[ix1];
    let g00 =
      Noise.gradients2D[Noise.hashTable[h0 + iy0] & Noise.gradientsMask2D];
    let g10 =
      Noise.gradients2D[Noise.hashTable[h1 + iy0] & Noise.gradientsMask2D];
    let g01 =
      Noise.gradients2D[Noise.hashTable[h0 + iy1] & Noise.gradientsMask2D];
    let g11 =
      Noise.gradients2D[Noise.hashTable[h1 + iy1] & Noise.gradientsMask2D];
    ...
    return lerp(lerp(v00, v10, tx), lerp(v01, v11, tx), ty) * Noise.sqrt2;
  }
}
